...
func GenerateDOTAndSVG(
    ...
    // Returns true if this package should get its own DOT/SVG file
    shouldWriteFile := func(pkg string) bool {
        if maxDepth == -1 {
            return true
        }
        d, ok := depthMap[pkg]
    ...
    processPkg := func(pkg, sanitized string, dg *DotGraph) {
        if !shouldWriteFile(pkg) {
            return // appears as cluster in other graphs, but no own file
        }
